---
summary: "격리나 iMessage가 필요한 경우 샌드박스 macOS VM (로컬 또는 호스팅)에서 OpenClaw 실행"
read_when:
  - 기본 macOS 환경에서 OpenClaw를 격리하려는 경우
  - 샌드박스에서 iMessage 통합 (BlueBubbles)을 원하는 경우
  - 복제 가능한 재설정 가능한 macOS 환경을 원하는 경우
  - 로컬 대 호스팅 macOS VM 옵션을 비교하려는 경우
title: "macOS VM"
---

# macOS VM에서의 OpenClaw (샌드박싱)
...
[Lume](https://cua.ai/docs/lume)을 사용하여 기존 Apple Silicon Mac에서 샌드박스 macOS VM으로 OpenClaw를 실행합니다.

다음을 제공합니다:

- 격리된 완전한 macOS 환경 (호스트는 깨끗하게 유지됨)
...
클라우드에서 macOS를 원한다면 호스팅 Mac 제공자도 사용 가능합니다:

- [MacStadium](https://www.macstadium.com/) (호스팅 Mac)
- 다른 호스팅 Mac 공급업체도 작동합니다. VM + SSH 문서를 따르십시오

SSH를 통해 macOS VM에 액세스하면 아래 6단계를 계속합니다.
...
이를 통해 macOS를 다운로드하고 VM을 생성합니다. VNC 창이 자동으로 열립니다.

참고: 연결 속도에 따라 다운로드가 시간이 걸릴 수 있습니다.
...
`youruser`를 생성한 계정으로, IP를 VM의 IP로 교체합니다.
...
온보딩 프롬프트에 따라 모델 제공자(Anthropic, OpenAI 등)를 설정합니다.
...
VM이 백그라운드에서 실행됩니다. OpenClaw의 데몬이 게이트웨이를 실행 상태로 유지합니다.
...
이것이 macOS에서 실행하는 핵심 기능입니다. [BlueBubbles](https://bluebubbles.app)를 사용하여 OpenClaw에 iMessage를 추가합니다.
...
게이트웨이를 재시작합니다. 이제 에이전트가 iMessage를 보내고 받을 수 있습니다.
...
진정한 상시 가동을 위해 전용 Mac mini 또는 소형 VPS를 고려하십시오. [VPS 호스팅](/vps) 참조.
...
| VM IP가 표시되지 않음        | VM이 완전히 부팅될 때까지 기다리고 `lume get openclaw`를 다시 실행         |
...
- [Docker 샌드박싱](/install/docker) (대안 격리 접근 방식)

---
title: "재시도 정책"
description: "아웃바운드 프로바이더 호출에 대한 재시도 정책"
---
...
- 멀티 스텝 흐름이 아닌 HTTP 요청별로 재시도합니다.
- 현재 단계만 재시도하여 순서를 유지합니다.
- 멱등성이 없는 작업의 중복을 피합니다.
...
- 일시적 오류 (429, 타임아웃, 연결/재설정/닫힘, 일시적으로 사용 불가)에서 재시도합니다.
- 사용 가능한 경우 `retry_after`를 사용하고, 그렇지 않으면 지수 백오프를 사용합니다.
- Markdown 파싱 오류는 재시도하지 않습니다. 일반 텍스트로 폴백합니다.

## 구성

`~/.openclaw/openclaw.json`에서 프로바이더별 재시도 정책을 설정하십시오:

---
summary: "`secrets apply` 플랜 계약: 대상 유효성 검사, 경로 매칭, `auth-profiles.json` 대상 범위"
read_when:
  - `openclaw secrets apply` 플랜 생성 또는 검토 시
  - `Invalid plan target path` 오류 디버깅 시
  - 대상 유형 및 경로 유효성 검사 동작 이해 시
